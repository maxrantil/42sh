...
/*   Updated: 2023/02/20 07:48:19 by jniemine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	logical_split(t_token *tokens, t_treenode *logical_op, \
		int start_of_left_cmd, int semicol)
{
					...
					int idx_logical_op, int semicol)
{
	...
	((t_logicalop *)logical_op)->left
		= create_command_tree(tokens, idx_logical_op + 1, \
			start_of_left_cmd + 1);
	logical_split(tokens, logical_op, start_of_left_cmd, semicol);
	return (logical_op);
}
...
t_treenode	*create_logical_op_tree(t_token *tokens, int i_tok, int semicol)
{
	...
	if (logical_op >= 0)
	{
		ft_printf("i_tok: %d logical_op: %d\n", i_tok, logical_op); //Well, WTF? We are losing our first 
		logicalptr = init_logical_op(logical_op_type(tokens, logical_op));
		((t_logicalop *)logicalptr)->left = create_command_tree(tokens, i_tok, logical_op);//parse_left_cmd(tokens, i_tok);
